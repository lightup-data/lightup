#!/usr/bin/env bash
# Lightup Gemini CLI Plugin — MCP connection script.
#
# Discovers lightup-api-credential.json, parses credentials, infers the MCP
# server URL, and starts an mcp-remote bridge (STDIO → HTTP).
#
# Overrides:
...
# app.X.lightup.ai  →  https://mcp.X.lightup.ai
# ---------------------------------------------------------------------------
infer_mcp_server() {
    local host="$1"
...
HTTP_URL="${MCP_SERVER}/mcp?host=${HOST}&refresh_token=${TOKEN}"

info "Connecting to ${MCP_SERVER}"

exec npx --yes --quiet mcp-remote "$HTTP_URL" 2>/dev/null
